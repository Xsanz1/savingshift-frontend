...
function Analytics() {
  ...
  const AddDataAnalytics = async (event) => {
    ...
    try {
      const postData = await axios.post(`http://localhost:8080/create/analytics`, {
        ...
        selisih: newPemasukan - newPengeluaran,
      });
  ...
  const UpdateDataAnalytics = async (event) => {
    event.preventDefault();
    try {
      console.log("Updating data:", id_analytics, saldo_awal, pemasukan, pengeluaran, selisih);
      ...
      console.log("Response from server:", putData.data);
  ...
  const DeleteDataAnalytics = async (event) => {
    ...
    const confirmDelete = window.confirm("Apakah Anda yakin menghapus data ini?");
    if (confirmDelete) {
      try {
        const deleteData = await axios.delete(
          // Mengirim permintaan penghapusan ke URL yang sesuai dengan id_analytics
                ...
                <Form.Group className="mb-3" controlId="exampleForm.ControlInput3">
                  ...
                  <Form.Control
                    ...
                    onChange={(e) => {
                      ...
                      // Calculate and set the selisih when pemasukan is changed
                      setSelisih(e.target.value - newPengeluaran);
                    }}
                    value={newPemasukan}
                  />
                </Form.Group>
                <Form.Group className="mb-3" controlId="exampleForm.ControlInput4">
                  ...
                  <Form.Control
                    ...
                    onChange={(e) => {
                      ...
                      // Calculate and set the selisih when pengeluaran is changed
                      setSelisih(newPemasukan - e.target.value);
                    }}
    ...
    {data_login.map((item, index) => {
      return (
        <CTableRow key={index}>
          ...
          <CTableHeaderCell> {item.pemasukan - item.pengeluaran} </CTableHeaderCell>
          <CTableDataCell>
            <CButton
